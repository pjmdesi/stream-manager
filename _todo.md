...
6. The "Open" item in the taskbar icon menu should be above the exit menu, all the non-interactive info should be above that.
7. Remove the "change" button from the stream items page which changes the root streams directory. I don't see users needing to do this frequently and it's a slightly dangerous process anyway, so it shouldn't be easily accessible. They can already change it in the settings, which is the more appropriate place for it.
8. Add some functionality to the series section item in the info side of the action panel for stream items. Allow the user to see a list of all the other episodes in a tooltip similar to the one that opens for the video counter cell. This one should open to the right, and the user should be able to enter the tooltip. It will display all the episodes in the series (format: [episode number]: [stream item title], including the one selected, which should be highlighted and not clickable) and allow the user to jump to that item when they click on it inside the tooltip.
