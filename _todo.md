...
5.  Bulk editing of stream items. For instance, let's say the streamer has finished a game and they want to now add the "{total_episodes}" merge tag to all the stream items in that season. They could select all the items for that season and then have the option to bulk edit the title template for all those items at once. SM would swap the templates for all those streams, update the titles, and then offer to push the changes to YouTube for all those streams at once as well.
6.  Maybe add the ability to connect and use other AI services in the integrations page besides Claude.
7.  Add the ability for SM to create a new stream item based on an already existing YouTube LIvestream, broadcast, or regular video. It will read and fill in the details for the stream item based on the YouTube video's details. This would greatly help out new users who already have a lot of videos. It would be good to be able to batch this process as well. Might run into API quotas. Need to discuss.
8.  Add the ability for users to create stream items by pulling videos/VODs directly from YouTube. This would be a great way for new users to quickly populate their stream list with existing content. The user would need to connect their YouTube account and then they could select from a list of their existing videos or livestreams to create stream items from or download all at once (for initial population). This would pull in the details for each video and fill in the stream item details based on that, including the thumbnail. This would probably be a modal found in the YouTube section of the integrations page. SM would create new folders in the root dir for each stream based on the upload/stream date & add any necessary files (like the thumbnail) to those folders.
9.  Add shorts upload functionality. Needs to be able to upload to YouTube.
10. Add "clip on twitch" functionality. This is a bit more complex than the YouTube one, because Twitch clips are created through an API call that takes a start and end time, and then Twitch processes the clip and makes it available after a few minutes. We would need to have some kind of system for checking the status of the clip creation and updating the app when it's ready. We also need to make sure the user knows the limitations of twitch clips.
...
15. Add multiselect functionality to the stream item details sidebar files section.
16. Add ability to double click slider controls like the ones used for the filters in the thumbnail editor properties panel. Double clicking on the knob of the slider will reset the value to the default for that filter.
17. Need to go through the onboarding process to make sure it's smooth and makes sense for all the new features we've added since th last time it was updated.
18. The layout for the auto-rules page needs to be updated. The sidebar layout does not match the rest of the app, it should be to the right of the sidebar, not below it.
