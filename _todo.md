...
1. Add multiselect functionality to the stream item details sidebar files section.
2. Need to go through the onboarding process to make sure it's smooth and makes sense for all the new features we've added since th last time it was updated.
...
8.  Add the ability for SM to create a new stream item based on an already existing YouTube LIvestream, broadcast, or regular video. It will read and fill in the details for the stream item based on the YouTube video's details. This would greatly help out new users who already have a lot of videos. It would be good to be able to batch this process as well. Might run into API quotas. Need to discuss.
