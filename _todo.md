...
9.  Allow axis constraint when moving objects in the thumbnail editor while the shift key is held, to make it easier to keep objects aligned while moving them around.
10. Attach cropping position to different clip regions so users can change where the crop is for those different regions. This will allow users to have more control over the cropping of their clips, and make it easier to ensure that important parts of the video are not cropped out when creating clips for highlights or shorts.
11. Add more cropping options like square, zooming in with any crop shape, and maybe just allow manual resizing of the crop box instead of just the preset aspect ratios. This will give users more flexibility in how they want to crop their videos for different platforms and purposes.
12. Add "save as template" to the metadata modal template selector, so that if the user enters in a value that doesn't match an existing template, they can click "add template" to add it as a new template option in the future. This will make it easier for users to create and save their own custom templates for use in the future without having to go to a separate settings page to add them.
13. Add ability to detect full number of episodes in a series of matching stream items. Then offer the user a merge field in the title and description templates for "current episode / total episodes" so that they can easily show the progress of a series of streams in their titles and descriptions.
14. In the thumbnail carousel on the stream page, we need to show a loading indicator while the thumbnail is loading from the cloud. We should have a default graphic that shows while the thumbnail is loading, and then replace it with the actual thumbnail once it's loaded. This will improve the user experience by giving them feedback that the thumbnail is loading, instead of just showing a blank space or a broken image icon.
15. Implement a recreation of the OBS stream picker for YouTube, to allow selection of scheduled streams through OBS. The ultimate goal would be to create a new stream item in the app, and when it's time to stream, the app automatically sends OBS the details for the YouTube connection. Here is what Claude recommended:
    ```
