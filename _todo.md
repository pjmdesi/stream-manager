...
2. When I tried to create a new stream item that has the same date as one that is already in the list, the app allowed me to click "Create stream". It did not warn me that a stream item already existed (which it should do as soon as the date is selected in the date picker). If the date is the same as an existing stream item, the app should show a warning and not allow the user to create the stream item until they select a different date that does not already have a stream item.