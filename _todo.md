...
5. More merge fields for use in the stream item details:
...
8.  Bulk editing of stream items. For instance, let's say the streamer has finished a game and they want to now add the "{total_episodes}" merge tag to all the stream items in that season. They could select all the items for that season and then have the option to bulk edit the title template for all those items at once. SM would swap the templates for all those streams, update the titles, and then offer to push the changes to YouTube for all those streams at once as well.
9.  Maybe add the ability to connect and use other AI services in the integrations page besides Claude.
...
18. Add multiselect functionality to the stream item details sidebar files section.
19. Add ability to double click slider controls like the ones used for the filters in the thumbnail editor properties panel. Double clicking on the knob of the slider will reset the value to the default for that filter.
20. Need to go through the onboarding process to make sure it's smooth and makes sense for all the new features we've added since th last time it was updated.
21. The layout for the auto-rules page needs to be updated. The sidebar layout does not match the rest of the app, it should be to the right of the sidebar, not below it.

## Bugs

1. The window buttons for the app while first launching are not in the correct place. Once it loads, the buttons shift to the correct position.
