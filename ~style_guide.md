...
A container that holds drag sources or drop targets and can become scrollable (the layers panel, a reorderable list, the files grid) must scroll itself while a drag hovers near its top or bottom edge, faster the closer the pointer gets, so every row is reachable without dropping halfway. Chromium does not do this for overflow containers on its own. `hooks/useDragAutoScroll.ts` provides it: pass the scroll container's ref and it attaches native listeners, no changes to the rows' own handlers (first user: the layers panel). Scrolling with the wheel while a drag is in progress must also stay possible. In the layers panel the wheel did nothing during a native `draggable` drag (observed 2026-09-16, under investigation: if it holds for every drag surface in the app it is the drag session itself, and a list that needs the wheel mid-drag has to use a pointer-event drag instead of `draggable`). Treat the wheel as the requirement when designing a new drag surface, and the edge auto-scroll as the floor every drag surface meets.
