...
- **Native select skin (rule, APP-29).** Every `<select>` carries `select-themed` (`assets/index.css`): it removes Chromium's menulist chrome, whose built-in inner padding and arrow made select text sit further in than an Input's, and draws the chevron as a background so no wrapper is needed. Set the padding explicitly to match the neighboring fields: `pl-3 pr-9 py-2` beside Inputs (`text-sm`), `pl-2 pr-7 py-1` beside NumberInputs (`text-xs`). Do not add a ChevronDown overlay on top of it.
...
**Angle fields (rule, 2026-09-15).** Every field that holds an angle (layer rotation, gradient angle, conic start) follows one contract. Committed values are wrapped into `0 ≤ a < 360` (`normalizeAngle` in `lib/liveTransform.ts`, passed as NumberInput's `wrap` prop). The wrap applies to every commit, typed or stepped, while the field keeps showing the text as typed until it loses focus (NumberInput's draft): the user can clear the field, type a minus sign, or type 400 and see it read 40 once they leave, and the stored value was 40 all along, so the entry is one undo step. Any number field must accept any typed number and make sense of it; it must never fight the keystrokes. Spinners and arrow keys never stop at a bound: no `min`/`max` on an angle NumberInput, the wrap does the work, so stepping up from 359 lands on 0. During a canvas gesture the live readout and the live panel value show the raw accumulated angle (spin a layer twice and read 720°), and the commit on release wraps it. Display precision is two decimals, like position and size (THU-27). Fields that are angles in name only (a percentage radius, an opacity) keep their real bounds.
...
- **Trigger wrapper is `flex` by default (APP-27, 2026-09-10).** Block-level, so it never sits on a line box. Inside flex and grid parents, which is where almost every Tooltip lives, this is identical to the old `inline-flex`. Two cases need `triggerClassName`: a Tooltip whose parent is a plain block gets `flex w-fit` (otherwise the trigger spans the row and the bubble anchors to the row's center; sites today: the Integrations setup-guide button, the expanded Auto-Rules watcher button in the nav, the wizard's dev-only reset), and a positioned badge carries its absolute classes on the trigger itself rather than on the child (the player's zoom badge, the stale-thumbnail badge in the streams list). `inline-flex` is for a trigger inside running text; no such site exists as of the sweep. History: until 2026-09-10 the default was `inline-flex`, and inside block parents it inherited the parent's line-height as a strut, making wrapped controls 1px taller than their siblings (the crop dropdown, the marker triangles, the Split Segment button). If a control is ever again 1px off its row, check the trigger's display first.

### Drag and drop in scrollable containers (rule, 2026-09-16)

A container that holds drag sources or drop targets and can become scrollable (the layers panel, a reorderable list, the files grid) must scroll itself while a drag hovers near its top or bottom edge, faster the closer the pointer gets, so every row is reachable without dropping halfway. Chromium does not do this for overflow containers on its own. `hooks/useDragAutoScroll.ts` provides it: pass the scroll container's ref and it attaches native listeners, no changes to the rows' own handlers. Users: the layers panel, the converter and combine page scrollers, the stream detail sidebar body. Scrolling with the wheel while a drag is in progress must also stay possible. Chromium delivers no wheel events during a native `draggable` drag (confirmed 2026-09-16 on two surfaces), so a list that needs the wheel mid-drag has to use a pointer-event drag instead of `draggable` (APP-40 plans that for the long lists). Treat the wheel as the requirement when designing a new drag surface, and the edge auto-scroll as the floor every drag surface meets.

### Anchored popovers (rule)

Any floating popup anchored to a point in the UI (marker edit popup, track color picker, clip handle popups, dropdown panels) **must keep itself fully inside its clipping container before it shows**: the viewport for portalled popovers, or the nearest `overflow-hidden` ancestor for inline ones. A popover that renders half-cut at a container edge is a bug, not a cosmetic nit (codified 2026-09-05 from the marker edit popup clipping at the timeline's ends). `<Tooltip>` already does this (side fallback + fit logic); it is the default choice for hover info, including `interactive` for hoverable content. Write a bespoke popover only when it needs focusable controls (inputs, swatch grids), and give it the clamp: measure after mount in a `useLayoutEffect` and shift the panel so it fits, keeping the logical anchor position (reference implementation: `MarkerEditPopup` in PlayerPage, which clamps against the strips wrapper via a `boundsRef`). For the common case, an inline dropdown menu under a button (`absolute top-full`), `hooks/useDropdownPlacement.ts` does the check: it returns `below` or `above` from the anchor and menu refs, and the caller swaps `top-full mt-*` for `bottom-full mb-*` (first user: the Assets options menu, 2026-09-16). Several older menus (the streams page filter menus, the date picker popup) still open below unconditionally and should adopt it.
...
9. **Tooltip trigger default**: resolved 2026-09-10 (APP-27), the default is `flex`; see the Tooltip entry. Kept here until the change has been through a release sweep.
