...
- The reschedule dialog's second step (push the new date to YouTube and Twitch) no longer closes on its own a second after the rename. It stays open until you confirm, skip, or dismiss it, so the push you ticked actually runs.
...
- Rotation snapping: hold Ctrl while dragging the rotate handle to snap to 90 degree steps, or Shift for 5 degree steps. While you move, resize, or rotate a layer, a readout beside the pointer shows the live position, size, or angle, and the transform fields in the properties panel update live as well. Rotating also no longer stretches the layer when the smart-snap guides light up.
- A Pixel snap toggle sits beside the grid and smart snap buttons, on by default. While it is on, moving and resizing a layer lands on whole pixels, so the numbers you see are the numbers you get; an image with its aspect ratio locked keeps the ratio exact even when that means a fractional height. Turn it off for free sub-pixel placement. The position, size, and rotation fields in the properties panel now show two decimals where there are any, and a value you type is kept exactly as typed. The arrow keys and spinners in those fields step to the next whole pixel while pixel snap is on and keep the fraction while it is off.
- The alignment and flip buttons have moved out of the top toolbar and into a small panel that appears under the selection on the canvas, the same way the crop controls sit under the crop region in the player. It shows as soon as one layer is selected, with the align-to-artboard and align-to-first-selected switch unlocking once two or more are selected, and steps aside while you drag, resize, or rotate. The toolbar is shorter as a result, and the stream title up there now has a tooltip with the full title and links back to the stream on the streams page.
- The editor's top bar has the same previous and next stream buttons as the player, plus previous and next episode and a jump-to-episode list for streams in a series, so you can move through thumbnails without going back to the streams page. Ctrl with the up and down arrows walks streams and Ctrl+Shift walks episodes, as on the other pages.
