...
- After a combine finishes, the source rows are replaced by a single completed item using the converter's done-job design: thumbnail, green check + output filename with the output's real metadata (codec, resolution, frame rate, duration, size) where the converter shows its preset, the owning stream link, a filled progress bar, a 100% / elapsed / output-location line (click the location to open the folder), and a Remove action behind the divider. No more copying the path out of a status line to find your file. A drop zone below it starts the next combine.
