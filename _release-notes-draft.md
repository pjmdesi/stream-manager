...
- Color palette panel in the editor sidebar: default swatch set to start, add your own with + (or promote a recently-used color from the recents row), remove on hover. Apply colors by dragging a swatch onto any color field, or keyboard-friendly via the new palette button on every color field (popover with palette + recents). The palette is saved beside your library (`_palette.json`) so it follows your streams through cloud sync; recent colors are remembered per machine. (Phase 3 still to come: reorder, multi-select, import/export — reword then.)
