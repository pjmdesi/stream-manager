...
- Detail sidebar cleanup round: the header now leads with the stream title (close button beside it) with the date, episode navigation, and Archived controls on the row below; the footer's whole-stream Combine button is gone (the files grid's select mode is the deliberate way to pick combine inputs), leaving Archive and Delete with permanent labels.
- The Reschedule and New stream / New episode dialogs now show their calendar permanently, a single compact panel with the calendar above and the typed date entry below, instead of hiding it behind a calendar icon click. The Reschedule dialog itself is much narrower to match.
