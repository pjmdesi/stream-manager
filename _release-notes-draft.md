...
- After a combine finishes, the source rows are replaced by a single green output card (like the converter's completed items): thumbnail, filename, and the output's codec, resolution, frame rate, duration, and size, with show-in-folder and clear actions. No more copying the path out of a status line to find your file. A drop zone below it starts the next combine.
