...
- Clip export now works on recordings that carry a chapter track, such as OBS's Hybrid MP4 output. It used to fail before encoding started, because the chapter track was copied into an intermediate file format that can't hold it.
...
- Clicking the zoomed timeline near its edges now just places the playhead. It used to also page the view sideways when the click landed inside the auto-scroll margin, so the spot you clicked jumped away. The view still follows the playhead during playback, paging once it reaches the visible edge after such a click.
...
- A "Last used" link under the font dropdown applies the font you last picked, no scrolling through the list. It remembers across sessions and hides while the layer already uses that font.
...
- Stored credentials are now encrypted at rest. The YouTube and Twitch tokens, both client secrets, and the Claude API key were previously plain text in SM's settings files; they are now encrypted with Windows' user-profile encryption (DPAPI, via Electron's safeStorage). Existing values migrate silently on first launch with no reconnect needed. If the OS ever can't decrypt them (for example a settings folder copied from another machine), SM treats the service as disconnected and offers a normal reconnect instead of failing. One heads-up: after running this version once, going back to an older SM version means the older build can't read the encrypted values, so YouTube, Twitch, and the Claude key would each need a one-time reconnect or re-entry there.
