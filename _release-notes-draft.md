...
- The alignment and flip buttons have moved out of the top toolbar and into a small panel that appears under the selection on the canvas, the same way the crop controls sit under the crop region in the player. It shows as soon as one layer is selected, adds the align-to-artboard and align-to-first-selected switch once two or more are selected, and steps aside while you drag, resize, or rotate. The toolbar is shorter as a result, and the stream title up there now has a tooltip with the full title and links back to the stream on the streams page.
