...
- [ ] Hydration thumbnails (app-wide): with a dehydrated file listed (combine row, converter queue, files grid), hydrate it by any route (pin on the streams page, or the combine run's download phase) → after the download finishes its thumbnail appears without remounting the page
