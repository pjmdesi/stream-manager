    ...
    body: (
      <>
        ...
        <ElementSection icon={<CircleHelp size={14} />} title="Recording setup for multi-track" anchorId="multitrack-setup">
          ...
          <p><strong className="text-gray-300">Track names and MP4.</strong> MP4 has no field for a track name, so the names from OBS's Audio tab only reach MKV recordings; an MP4 arrives with unnamed tracks. Fill in <em>Default audio track names</em> under Settings &gt; Video Player to match your OBS track order and SM labels unnamed tracks with them. A name stored in the recording always wins over the default, and you can rename any track for one file by double-clicking its name in the track row; a small icon beside the name says which of the three it is.</p>
        </ElementSection>
        ...
        <ElementSection icon={<Scissors size={14} />} title="Clip mode">
          ...
          <ul className="list-none pl-0 flex flex-col gap-1.5">
            <li><strong className="text-gray-300">Segments</strong>: press <Kbd>A</Kbd> or click the <em>Add Segment</em> button to add a clip segment centered on the playhead. Drag the in/out handles to refine; click a handle for a precise timecode input. Hold <Kbd>Alt</Kbd> while dragging a handle to move both ends at once and keep the segment centered. Multiple segments are concatenated into one export. <Kbd>S</Kbd> splits the segment under the playhead.</li>
