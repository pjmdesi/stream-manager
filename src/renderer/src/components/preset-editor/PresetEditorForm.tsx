...
interface Props {
  ...
  onCancel: () => void
  /** Edit mode: form fields prefill from the preset's customForm; if the
   *  preset doesn't have one (e.g. raw or imported), only the Advanced view is
   *  available. Omit / null for a new preset. */
...
/**
 * PresetEditorForm — the custom-preset create/edit form, rendered INLINE
 * (inside the Presets management modal) rather than as its own modal, so we
 * don't stack a modal on a modal. Mirrors the inline-form pattern used by the
 * Templates modal. Fields are paired into two columns where it helps the tall
 * form fit.
 */
export function PresetEditorForm({ onSave, onCancel, editing }: Props) {
  // ── Form state ──────────────────────────────────────────────────────────
  ...
  // Hoisted so the hook is called unconditionally (the textarea is rendered
  // conditionally under the Advanced toggle).
  const rawArgsRef = useAutoGrowTextarea(rawArgs)

  // ── Encoder availability ────────────────────────────────────────────────
  // Hardware-probed (not just "compiled into the binary") — main process
  // invokes each GPU encoder against a synthetic frame and reports which ones
  // actually work. Result is cached in main + pre-warmed at app startup, so by
  // the time the form mounts it's almost always instant. CPU is always
  // assumed available.
  ...
  // Track whether the currently-selected encoder is the auto-pick (true) or a
  // user override (false). Drives the "Auto-picked best available" hint.
  ...
  // availability finishes loading). NVENC > QSV > AMF > CPU. Skipped when
  // editing — respect the saved choice.
  useEffect(() => {
  ...
  // Even when editing or after a manual pick, make sure the chosen encoder is
  // actually available; if not (preset authored on another machine), fall back
  // to CPU.
  useEffect(() => {
    if (form.video.codec === 'copy') return
  ...
  // actually changes. Snapping makes every nudge produce a visible difference.
  ...
  // Sync rawArgs <- compiled whenever the form changes and we're not in
