...
export function TemplateBodyEditor({
  value, onSave, placeholder, knownKeys, inapplicableKeys, resolvedValues, tabAttached, tabActive, multiline, minHeight, insertRef, autoFocus, height, onHeightChange, aiFetcher, onAiReject, attachLeft,
}: {
  value: string
  ...
  /** Square the LEFT corners (keeping the full border as the seam) so a
   *  control can attach flush to the editor's left edge — the Twitch
   *  title override pill. Composable with tabAttached (top-right). */
  attachLeft?: boolean
}) {
  ...
  const cornerCls = `rounded-lg${tabAttached ? ' rounded-tr-none' : ''}${attachLeft ? ' rounded-l-none' : ''}`
