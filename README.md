...
- **Bulk offload to cloud** — when your streams folder lives inside a cloud sync root, multi-select stream sessions and free local disk space in one click. Uses the Windows Cloud Files API to evict file data without touching the cloud copy, so the files can re-download on demand later. The streams' primary thumbnails are automatically kept local so the streams list never re-hydrates them on render. A live progress modal lists every file with its size and status, auto-scrolls to the file currently being processed, verifies each eviction actually happened, reports the running total of space saved, and supports mid-run cancellation.
- **Action Panel** — clicking any stream row expands an inline panel underneath with the full action button set (Player, Converter, Combine, Open folder, Reschedule, etc.), plus supplementary metadata that doesn't fit in the row: series episode number with total, YouTube/Twitch titles when they differ from the row title, full description on hover, and tags. Hosts the **New Episode** duplicate action and a single-item Archive option that bypasses selection mode.
- **New Episode** — clone an existing stream as the next episode in its series. Stream type and games carry over, source thumbnails copy automatically (built-in editor JSON+PNG **or** external image files — both handled the same way), and the metadata modal opens pre-filled with date defaulted to today.
...
- **Layer effects** — full Konva filter library on image layers (brightness, contrast, hue, saturation, luminance, blur, pixelate, posterize, enhance, threshold, grayscale, sepia, invert, emboss) with sliders + toggles, and a master enable switch that preserves slider values for A/B comparison. Drop shadow available on every layer type with color, offset, blur, and opacity controls.
